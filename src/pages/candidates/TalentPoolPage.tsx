...
const TalentPoolPage: FC = () => {
 ...
 // --- REFACTORED: Main data fetching logic ---
   ...
   // NEW: Query to fetch jobs for the popover
...
const renderCandidateList = () => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      {/* MODIFIED HEADER: Add a column for Match Score if a job is selected */}
      <div className="grid grid-cols-12 gap-3 px-4 py-3 bg-gray-50/80 border-b border-gray-200">
        <div className={`${selectedJob ? 'col-span-3' : 'col-span-3'} text-xs font-semibold text-gray-500 uppercase tracking-wider`}>Candidate</div>
        {selectedJob && (
        ...
        <div className="col-span-1 text-xs font-semibold text-gray-500 uppercase tracking-wider">Suggested Title</div>
        <div className={`${selectedJob ? 'col-span-3' : 'col-span-3'} text-xs font-semibold text-gray-500 uppercase tracking-wider`}>Added By</div>
      </div>
        ...
        {paginatedCandidates.map((candidate) => {
          ...
          return (
            <div key={candidate.id} className="grid grid-cols-12 gap-3 items-center px-4 py-4 transition-all duration-300 ease-in-out hover:bg-gray-50">
              ...
              {/* CANDIDATE NAME & PROFILE STATUS - 3 columns */}
              <div className={`${selectedJob ? 'col-span-3' : 'col-span-3'}`}>
                <div className="flex items-center gap-3">
                  <Link 
